...
static const int COPY_DELAY =  1000;
...
#define GIT_C_BEGIN "git c"
#define GIT_C_TRIM "[Heckout/Ommit]"

#define GIT_R_BEGIN "git re"
#define GIT_R_TRIM "[Set/Vert/Base]"

#define GIT_S_BEGIN "git s"
#define GIT_S_TRIM "[taSh/How/taTus]"
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case G_C:
      if (record->event.pressed) {
        send_string_remembering_length(GIT_C_BEGIN GIT_C_TRIM);
        layer_on(GIT_C);
      }
      break;
    case G_CHEC:
      if (!record->event.pressed) {
        ...
        press_n_times(strlen(GIT_C_TRIM), KC_BACKSPACE);
        char *git_checkout = "heckout ";
        send_string_with_translation(git_checkout);
        char_to_bspace = strlen(GIT_C_BEGIN) + strlen(git_checkout);
        if (shifted) {
          char *git_checkout_shifted = "-b ";
          send_string_with_translation(git_checkout_shifted);
          char_to_bspace = strlen(GIT_C_BEGIN) + strlen(git_checkout) + strlen(git_checkout_shifted);
        }
        layer_off(GIT_C);
      }
      break;
    case G_COMM:
      if (!record->event.pressed) {
        ...
        press_n_times(strlen(GIT_C_TRIM), KC_BACKSPACE);
        char *git_commit = "ommit ";
        send_string_with_translation(git_commit);
        char_to_bspace = strlen(GIT_C_BEGIN) + strlen(git_commit);
        layer_off(GIT_C);
    ...
    case G_DEV:
      if (record->event.pressed) {
        send_shifted_strings(DEFAULT_GIT_BRANCH, "develop");
      }
      break;
    case G_DIFF:
      if (record->event.pressed) {
    ...
    case G_HEAD:
      if (record->event.pressed) {
        send_string_remembering_length("HEAD~");
      }
      break;
    case G_LOG:
      if (record->event.pressed) {
    ...
    case G_MERG:
      if (record->event.pressed) {
        send_shifted_strings_add("git merge ", "--abort");
      }
      break;
    case G_PULL:
      if (record->event.pressed) {
        send_shifted_strings_add("git pull ", "origin " DEFAULT_GIT_BRANCH);
      }
      break;
    case G_PUSH:
      if (record->event.pressed) {
    ...
    case G_R:
      if (!record->event.pressed) {
        send_string_remembering_length(GIT_R_BEGIN GIT_R_TRIM);
        layer_on(GIT_R);
      }
      break;
    case G_RBASE:
      if (!record->event.pressed) {
        ...
        press_n_times(strlen(GIT_R_TRIM), KC_BACKSPACE);
        char *git_rebase = "base ";
        send_string_with_translation(git_rebase);
        char_to_bspace = strlen(GIT_R_BEGIN) + strlen(git_rebase);

        if (shifted) {
          send_string_with_translation("-i ");
          char_to_bspace = strlen(GIT_R_BEGIN) + strlen(git_rebase) + 3;
        }
    ...
    case G_RVERT:
      if (!record->event.pressed) {
        press_n_times(strlen(GIT_R_TRIM), KC_BACKSPACE);
        char *git_revert = "vert ";
        send_string_with_translation(git_revert);
        char_to_bspace = strlen(GIT_R_BEGIN) + strlen(git_revert);
        layer_off(GIT_R);
      }
      break;
    case G_RST:
      if (!record->event.pressed) {
        ...
        press_n_times(strlen(GIT_R_TRIM), KC_BACKSPACE);
        char *git_reset = "set ";
        send_string_with_translation(git_reset);
        char_to_bspace = strlen(GIT_R_BEGIN) + strlen(git_reset);

        if (shifted) {
          send_string_with_translation("--");
          char_to_bspace = strlen(GIT_R_BEGIN) + strlen(git_reset) + 2;
        }
        layer_off(GIT_R);
      }
      break;
    case G_S:
      if (!record->event.pressed) {
        send_string_remembering_length(GIT_S_BEGIN GIT_S_TRIM);
        layer_on(GIT_S);
      }
      break;
    case G_SHOW:
      if (!record->event.pressed) {
        press_n_times(strlen(GIT_S_TRIM), KC_BACKSPACE);
        char *git_show = "how ";
        send_string_with_translation(git_show);
        char_to_bspace = strlen(GIT_S_BEGIN) + strlen(git_show);
        layer_off(GIT_S);
      }
      break;
    case G_STSH:
      if (!record->event.pressed) {
        ...
        press_n_times(strlen(GIT_S_TRIM), KC_BACKSPACE);
        char *git_stash = "tash ";
        send_string_with_translation(git_stash);
        char_to_bspace = strlen(GIT_S_BEGIN) + strlen(git_stash);

        if (shifted) {
          clear_mods();
          char *git_stash_pop= "pop ";
          send_string_with_translation(git_stash_pop);
          char_to_bspace = strlen(GIT_S_BEGIN) + strlen(git_stash) + strlen(git_stash_pop);
        }
    ...
    case G_STAT:
      if (!record->event.pressed) {
        press_n_times(strlen(GIT_S_TRIM), KC_BACKSPACE);
        char *git_status = "tatus ";
        send_string_with_translation(git_status);
        char_to_bspace = strlen(GIT_S_BEGIN) + strlen(git_status);
        layer_off(GIT_S);
      }
      break;
